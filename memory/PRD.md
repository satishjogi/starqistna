...
## Implemented (2026-08-16) — Ungray + Shared Seat Pool via `bus_instance_id` (Option C)
- **Reversal of hard-forced from/to**: Admin schedule form no longer greys out the From/To fields when a route is linked. Admin can create as many sibling schedules as they want per physical bus (KL→Melaka + KL→JB + Melaka→JB, all on the same 09:00 bus) and each shows as its own direct-point row in search results.
- **`bus_instance_id`** — new field on `schedules` (unique per physical bus = same `route_id + date + time`). Legacy/unlinked schedules use their own `id` (unchanged behaviour). Startup migration `_migrate_bus_instance_ids` backfills every schedule + every seat_lock.
- **Seat lock DB constraint** — dropped legacy `uniq_schedule_seat_active`, replaced with `uniq_bus_instance_seat_active` on `(bus_instance_id, seat_number)`. Ensures **seat 5A locked on the KL→Melaka sibling automatically fails the lock attempt on the KL→JB sibling** — cross-sibling atomic protection at the DB level.
- **Search + schedule detail** — seat availability now grouped by `bus_instance_id` so `seats_available` is shared across siblings. Seat map on `GET /schedules/{id}` shows every cross-sibling booking too, so a customer viewing the KL→JB sibling sees seat 5A pre-taken if it was booked on the KL→Melaka sibling of the same bus.
- **Admin form UX** — pickers narrow to the route's stops only, defaults populate to route origin/destination (still editable), and a fresh info banner explains shared seat pool behaviour. Duplicate-exact-segment guard still enforced (`route_id + date + time + from + to` uniqueness) — different from/to combos allowed, only the same segment twice is rejected.
- **Tests** — refactored `test_route_schedule_segments.py`: creates two siblings on the same route+date+time, verifies bus_instance_id sharing, cross-sibling lock rejection (409), search seats_available parity, cross-sibling seat-map visibility, and legacy schedule isolation. **19/19 pytest cases green**.

## Implemented (2026-08-15) — Route ↔ Schedule ↔ Bookings unified as ONE physical bus
- **Model**: A Route defines the exact ordered chain of stops a bus makes (KL Sentral → Melaka → JB → GMC), with pickup/drop-off flags per stop and a pairing fare matrix. A Schedule linked to a Route represents ONE physical bus running that route on a specific date/time. All valid (pickup, drop-off) combos from the route become bookable segments — but they all use the same schedule's seat map.
- **Admin schedule form**: When a route is linked, From/To fields are auto-forced to the route's origin/final-destination and locked (grey, disabled). An info banner explains "This schedule represents ONE physical bus. Seats are SHARED across every segment." Backend enforces the same on create/bulk-create — attempting to override from/to is silently corrected to route origin/destination.
- **DB constraint**: New partial unique index `uniq_route_departure` on `schedules.(route_id, date, time)` — prevents accidentally re-creating the "one physical bus split into multiple pair rows" mistake at the database level.
- **Search endpoint**: `/api/search` now matches route-linked schedules whose route pairings cover the requested (pickup, drop-off) pair, even when it's not the schedule's own origin→destination. Each schedule surfaces ONCE with the segment's specific pickup/drop-off and pairing fare embedded. Duplicate suppression by schedule.id ensures a customer never sees the same physical bus twice.
- **Booking flow**: `POST /api/bookings` accepts optional `pickup_terminal_id` + `dropoff_terminal_id`. When provided (route-linked schedule + valid pairing), fare is pulled from the route's pairing matrix (not the schedule's own fare fields). The booking stores the SOLD segment on `from_terminal_id` / `to_terminal_id`, while the schedule keeps physical-bus endpoints — so tickets and emails always show the customer's specific journey.
- **Seat locking = full-trip block (Option A)**: Because one physical bus now maps to exactly one schedule row, the existing `(schedule_id, seat_number)` uniqueness naturally blocks seat 5A across every segment search on the same bus — booking Melaka→JB blocks the seat for anyone searching KL→SG, Melaka→SG, or JB→GMC on that same schedule.
- **Passengers page**: sends the specific pickup/drop-off IDs from the flow store so the segment fare is applied at checkout.
- **Tests**: new `test_route_schedule_segments.py` — full end-to-end pytest cases covering route creation with 6-pairing matrix, schedule creation forcing route origin/destination, duplicate rejection, mid-segment search discovery, segment-fare vs full-trip-fare pricing, and cross-segment seat blocking. All 18 tests (bus_types + cancel + this) still pass.
